...
class _NumberQuizScreenState extends State<NumberQuizScreen>
  ...
  Future<void> handleAnswer(int selectedIndex) async {
    ...
    if (isCorrect) {
      ...
      showAnimatedPopup(icon: Icons.star, title: "Correct!", subtitle: "You earned 20 XP${levels > 0 ? " & leveled up!" : ""}", bgColor: const Color(0xFF2C5CB0));

      if (levels > 0) {
        final newlyUnlocked = QuestStatus.unlockedBetween(oldLvl, QuestStatus.level);
        for (final key in newlyUnlocked) {
          showAnimatedPopup(icon: Icons.lock_open, title: "New Level Unlocked!", subtitle: QuestStatus.titleFor(key), bgColor: const Color(0xFFFF4B4A));
          await Future.delayed(const Duration(milliseconds: 300));
        }
      }
    } else {
      final correctValue = (questions[qIdx]['options'] as List<dynamic>)[correctIndex].toString();
      showAnimatedPopup(icon: Icons.close, title: "Incorrect", subtitle: "Correct: $correctValue", bgColor: const Color(0xFFFF4B4A));
    }
    ...
    if (_allAnsweredInSession()) {
      ...
      showAnimatedPopup(icon: Icons.emoji_events, title: "Quiz Complete!", subtitle: "Score: $sessionScore/${activeIndices.length}", bgColor: const Color(0xFF2C5CB0));
      ...
      if (isPerfect) {
        ...
        if (QuestStatus.canClaimQuest9()) {
          ...
          showAnimatedPopup(icon: Icons.stars, title: "Quest 9 Complete!", subtitle: "Perfect round! +200 keys", bgColor: const Color(0xFF2C5CB0));
      ...
      if (QuestStatus.numbersRoundsCompleted >= 3 && !QuestStatus.quest10Claimed) {
        if (QuestStatus.canClaimQuest10()) {
          ...
          await Future.delayed(const Duration(milliseconds: 500));
          showAnimatedPopup(icon: Icons.military_tech, title: "Quest 10 Complete!", subtitle: "3 rounds finished! +200 keys", bgColor: const Color(0xFFFF4B4A));
          await Future.delayed(const Duration(seconds: 2));
        }
      }
      ...
      if (justEarned && mounted) {
        showAnimatedPopup(icon: Icons.military_tech, title: "Medal unlocked!", subtitle: "Finish your first quiz", bgColor: const Color(0xFF2C5CB0));
        await Future.delayed(const Duration(seconds: 2));
      }

      final didIncrease = QuestStatus.addStreakForLevel();
      if (didIncrease && mounted) {
        showAnimatedPopup(icon: Icons.local_fire_department, title: "Streak +1!", subtitle: "Current streak: ${QuestStatus.streakDays} day${QuestStatus.streakDays == 1 ? '' : 's'}",  bgColor: const Color(0xFFFF4B4A));
        await Future.delayed(const Duration(seconds: 2));
      }
  ...
  void showAnimatedPopup({required IconData icon, required String title, required String subtitle, required Color bgColor}) {
    ...
    final entry = OverlayEntry(builder: (_) => Positioned(top: 60, right: 16, child: _NumberSlideInPopup(icon: icon, title: title, subtitle: subtitle, bgColor: bgColor)));
      ...
      children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              const Text("Number Level", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF2C5CB0), letterSpacing: -0.5)),
  ...
  Widget _buildQuestionCard(Map<String, dynamic> question) {
    return Container(
      ...
      child: Column(
        children: [
          const Text("What number is shown?", textAlign: TextAlign.center, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Color(0xFF2C5CB0), letterSpacing: -0.3)),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: Text(option, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: isSelected ? Colors.black : const Color(0xFF2C5CB0)))),
...
class _NumberSlideInPopup extends StatefulWidget {
  ...
  const _NumberSlideInPopup({required this.icon, required this.title, required this.subtitle, required this.bgColor});

  @override
  State<_NumberSlideInPopup> createState() => _NumberSlideInPopupState();
}

class _NumberSlideInPopupState extends State<_NumberSlideInPopup> with SingleTickerProviderStateMixin {
  late final AnimationController controller = AnimationController(duration: const Duration(milliseconds: 280), vsync: this)..forward();
  late final Animation<Offset> offsetAnimation = Tween<Offset>(begin: const Offset(1.1, 0), end: Offset.zero).animate(CurvedAnimation(parent: controller, curve: Curves.easeOut));
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.title, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)), Text(widget.subtitle, style: const TextStyle(color: Colors.white70, fontSize: 14))])),
            ],
          ),
        ),
      ),
